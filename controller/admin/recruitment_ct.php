...
require_once __DIR__ . '/../../logic/admin/recruitment_logic.php';
...
if ($data ['status']) {
	...
	// インスタンス生成
	$recruitment_ct = new recruitment_ct ();

	// コントローラー呼び出し
	$data = $recruitment_ct->main_control ( $_POST );
} else {
	// パラメータに不正があった場合
	// AJAX返却用データ成型
	$data = array (
...
class recruitment_ct {
	...
	public function __construct() {
		// 管理画面ユーザーロジックインスタンス
		$this->recruitment_logic = new recruitment_logic ();
	...
	public function main_control($post) {
		...
		if ($post ['method'] == 'init') {
			// 初期処理　HTML生成処理呼び出し
			$data = $recruitment_ct->create_data_list ( $post );
		} else if ($post ['method'] == 'entry') {
			// 新規登録処理
			$data = $recruitment_ct->entry_new_data ( $post );
		} else if ($post ['method'] == 'edit_init') {
			// 編集初期処理
			$data = $recruitment_ct->get_detail ( $post ['edit_del_id'] );
		} else if ($post ['method'] == 'edit') {
			// 編集更新処理
			$data = $recruitment_ct->update_detail ( $post );
		} else if ($post ['method'] == 'delete') {
			// 削除処理
			$data = $recruitment_ct->delete ( $post ['id'] );
		} else if ($post ['method'] == 'recovery') {
			// 有効化処理
			$data = $recruitment_ct->recovery ( $post ['id'] );
		} else if ($post ['method'] == 'private') {
			// 非公開化処理
			$data = $recruitment_ct->private_func ( $post ['id'] );
		} else if ($post ['method'] == 'release') {
			// 公開化処理
			$data = $recruitment_ct->release ( $post ['id'] );
		}
	...
	private function create_data_list($post) {
		...
		$list_html = $this->recruitment_logic->create_data_list ( array (
	...
	private function entry_new_data($post) {
		...
		// 登録ロジック呼び出し
		$this->recruitment_logic->entry_new_data ( array (
                	$post['title'],
	                $post['job_type'],
	...
	private function get_detail($recruitment_id) {
		...
		$data = array (
			'status' => true,
	                ...
	                'job_type' => $reult_detail['job_type'],
	                'job_description' => $reult_detail['job_description'],
                	'emp_status' => $reult_detail['emp_status'],
	                'work_place' => $reult_detail['work_place'],
	                'acad' => $reult_detail['acad'],
                	'salary' => $reult_detail['salary'],
	                'pay_raise' => $reult_detail['pay_raise'],
	                'bonus' => $reult_detail['bonus'],
                	'allowance' => $reult_detail['allowance'],
	...
	private function update_detail($post) {
		...
		// 編集ロジック呼び出し
		$this->recruitment_logic->update_detail ( array (
                	$post['title'],
	                $post['job_type'],
	                $post['job_description'],
                	$post['emp_status'],
	...
	public function recovery($id) {
		// 更新ロジック呼び出し
		$this->recruitment_logic->recoveryl_func ( $id );
	...
	public function delete($id) {
		// 更新ロジック呼び出し
		$this->recruitment_logic->del_func ( $id );
	...
	public function private_func($id) {
		// 更新ロジック呼び出し
		$this->recruitment_logic->private_func ( $id );
	...
	public function release($id) {
		// 更新ロジック呼び出し
		$this->recruitment_logic->release_func ( $id );
