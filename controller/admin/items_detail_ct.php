...
require_once __DIR__ . '/../../logic/admin/items_detail_logic.php';
...
if ($data ['status']) {
	...
	// インスタンス生成
	$items_detail_ct = new items_detail_ct ();

	// コントローラー呼び出し
	$data = $items_detail_ct->main_control ( $_POST );
} else {
	// パラメータに不正があった場合
	// AJAX返却用データ成型
	$data = array (
...
class items_detail_ct {
	...
	public function __construct() {
		// 管理画面ユーザーロジックインスタンス
		$this->items_detail_logic = new items_detail_logic ();
	...
	public function main_control($post) {
		...
		if ($post ['method'] == 'init') {
			// 初期処理　HTML生成処理呼び出し
			$data = $items_detail_ct->create_data_list ( $post );
		} else if ($post ['method'] == 'entry') {
			// 新規登録処理
			$data = $items_detail_ct->entry_new_data ( $post );
		} else if ($post ['method'] == 'edit_init') {
			// 編集初期処理
			$data = $items_detail_ct->get_detail ( $post ['edit_del_id'] );
		} else if ($post ['method'] == 'edit') {
			// 編集更新処理
			$data = $items_detail_ct->update_detail ( $post );
		} else if ($post ['method'] == 'delete') {
			// 削除処理
			$data = $items_detail_ct->delete ( $post ['id'] );
		} else if ($post ['method'] == 'recovery') {
			// 有効化処理
			$data = $items_detail_ct->recovery ( $post ['id'] );
		} else if ($post ['method'] == 'private') {
			// 非公開化処理
			$data = $items_detail_ct->private_func ( $post ['id'] );
		} else if ($post ['method'] == 'release') {
			// 公開化処理
			$data = $items_detail_ct->release ( $post ['id'] );
		}
	...
	private function create_data_list($post) {
		...
		$list_html = $this->items_detail_logic->create_data_list ( array (
	...
	private function entry_new_data($post) {
		...
		// 登録ロジック呼び出し
		$this->items_detail_logic->entry_new_data ( array (
            		$post['category'],
		        $post['items_id'],
		        $post['title'],
		        $post['lead'],
	...
	private function get_detail($items_detail_id) {
		...
		$_SESSION['items']['category'] = $reult_detail['category'];
		// AJAX返却用データ成型
		$data = array (
			'status' => true,
	                'category' => $reult_detail['category'],
	                'title' => $reult_detail['title'],
	                'items_id' => $reult_detail['items_id'],
	...
	private function update_detail($post) {
		// 編集ロジック呼び出し
		$this->items_detail_logic->update_detail ( array (
                	$post['category'],
                	$post['items_id'],
                	$post['title'],
	                $post['lead'],
	...
	public function recovery($id) {
		// 更新ロジック呼び出し
		$this->items_detail_logic->recoveryl_func ( $id );
	...
	public function delete($id) {
		// 更新ロジック呼び出し
		$this->items_detail_logic->del_func ( $id );
	...
	public function private_func($id) {
		// 更新ロジック呼び出し
		$this->items_detail_logic->private_func ( $id );
	...
	public function release($id) {
		// 更新ロジック呼び出し
		$this->items_detail_logic->release_func ( $id );
