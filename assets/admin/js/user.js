    ...
    call_ajax_edit_init = function (post_data){
        ajax.get(post_data).done(function(result) {
            ...
            if (result.data.status) {
                ...
                $('.role-selection').val(null).trigger('change');

                let roleId = result.data.role_id || null;
                $('.role-selection').val(roleId).trigger('change');

                let pwd = $("input[name='password']");
                let pwdc = $("input[name='password_conf']");

                if (pwd.hasClass('required')) {
                    pwd.removeClass('required');
                }

                if (pwdc.hasClass('required')) {
                    pwdc.removeClass('required');
                }

                pwd.parents('.formRow').find('span').hide();
                pwdc.parents('.formRow').find('span').hide();
                //ロード終了
                loaded();
...
$('[name=new_entry]').on('click', function(){
    ...
    if (!pwdc.hasClass('required')) {
        pwdc.addClass('required');
    }

    pwd.parents('.formRow').find('span').show();
    pwdc.parents('.formRow').find('span').show();
});
