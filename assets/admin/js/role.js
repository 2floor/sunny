...
$(function() {
    ...
    call_ajax_init = function (post_data, startPage = 1, afterChange = false){
        ...
        let group = cParams.get('group');

        let uri = new URLSearchParams(post_data).toString();
        $('#pagination-container').pagination({
            dataSource: $('#ct_url').val() + '?group_role='+(group || 1)+'&' + uri,
            locator: 'data.html',
    ...
    call_ajax_edit_init = function (post_data){
        ajax.get(post_data).done(function(result) {
            ...
            if (result.data.status) {
                ...
                let perms = result.data.perms || [];
                $.each(perms, function (index, value) {
                    let checkbox = $('input[name="perms[]"][value="'+value+'"]');
