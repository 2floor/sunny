...
class hospital_logic extends base_logic
{
    ...
    public function create_data_list($params, $search_select = null){
        ...
        for($i = 0; $i < count ($hospitals ?? []); $i ++) {
            ...
            $edit_html_b = '';
            if ($row ['public_flg'] == 1) {
                $edit_html_b .= "<a herf='javascript:void(0);' class='release btn btn-default waves-effect w-md btn-xs' name='release_" . $row ['id'] . "' value='" . $row ['id'] . "'>非公開</a>";
            } else {
                $edit_html_b .= "<a herf='javascript:void(0);' class='private btn btn-custom waves-effect w-md btn-xs ' name='private_" . $row ['id'] . "' value='" . $row ['id'] . "'>公開</a>";
            }
